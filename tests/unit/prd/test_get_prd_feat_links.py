...
class TestPRD(TestCase):
    ...
    def setUp(self):
        with open(settings.DATADIR.child("pathfinder").child("feats.html")) as fh:
    ...
    def test_get_prd_spell_links_return_contains_iron_will(self):
        ...
        self.assertEqual(spells["Iron Will"], "http://paizo.com/pathfinderRPG/prd/feats.html#_iron-will")

    def test_get_prd_spell_links_finds_adjuring_step(self):
        spells = get_prd_feat_links()
        self.assertIn("Twin Thunders", spells)
        self.assertEqual(spells["Twin Thunders"], "http://paizo.com/pathfinderRPG/prd/ultimateCombat/ultimateCombatFeats.html#_twin-thunders")
